...
const Play = () => {
    ...
    const placeTable = (row: number, col: number) => {
        const isEmpty = table[row][col] == ''
        const currentPlayer = table[row][col].split('_')[0];
        const currentSizeIdx = +table[row][col].split('_')[1];

        if (sizeIdx == -1) return;
        if (!isEmpty && (currentPlayer == player || sizes[sizeIdx] <= sizes[currentSizeIdx])) return;
    ...
    useEffect(() => {
        ...
        if (player == 'o' && winner == '') {
            ...
            table.forEach((row, _r) => {
                row.forEach((col, _c) => {
                    if (+col.split('_')[1] < sizes[botSelectedIdx]) {
            ...
            setTimeout(() => {
                let row = botSelectedPos.i
                let col = botSelectedPos.j
                let newTable = [...table]
                newTable[row][col] = `${player}_${botSelectedIdx}`
                setTable(newTable)

                console.log(table)
                let newUsedX = [...usedX]
                let newUsedO = [...usedO]
                newUsedO[botSelectedIdx] = true
                setUsedO(newUsedO)
    ...
    return (
        <div className="relative flex h-screen flex-col items-center justify-center bg-teal-500 overflow-hidden">
            ...
            <Script strategy="beforeInteractive" src="https://cdn.jsdelivr.net/pyodide/v0.20.0/full/pyodide.js"></Script>
            ...
            <animated.div style={fadeIn} className="">
                ...
                <div className="shadow-md" >
                    <div className={`w-full border-white h-24 border-4 border-b-0 flex items-center justify-between px-4 ${player == 'o' ? 'bg-teal-600' : 'bg-transparent'}`}>
                        ...
                        {sizes.map((size, _idx) =>
                            <div className={`flex flex-col items-center justify-end ${usedO[_idx] && 'invisible'}`} >
